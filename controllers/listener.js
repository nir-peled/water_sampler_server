...
class Listener {
	...
	#setup_user_crud() {
		...
		app.get("/user/update/:username", (request, response) => {
			...
			if (!user) response.redirect("/users");
			else response.render("edit_user", { username, edited_user: user });
		});
	...
	#setup_sensor_managing() {
		app.use("/sensors", (request, response, next) => {
			if (!request.user_data) this.#send_unautorized(response);
			else next();
		}); // use sensors
		...
		app.get("/sensors/all", (request, response) => {
			...
			this.#database
				.get_sensor_list()
				.then((sensors) => response.render("sensors_list", { sensors }))
				.catch((err) => {
					console.log(err); // debug
					response.status(SERVER_ERR);
					this.#redirect(response, "/", {
		...
		*/
		...
		app.get("/sensors/:sensor_name/config", (request, response) => {
			...
			let sensor_name = request.params.sensor_name;
			this.#database
				.get_sensor(sensor_name, { config: 1 })
				.then((sensor) =>
		...
		app.post("/sensors/:sensor_name/config.json", (request, response) => {
			...
			this.#sensor_manager
				.config_sensor(sensor_name, config_data)
				.then(() => this.#database.set_sensor_config(sensor_name, config_data))
				.then(() => {
				...
				.catch((err) => {
					...
					response
						.status(SERVER_ERR)
						.send(`Unable to configure Sensor ${sensor_name}: ${err.message}`);
				});
		}); // post sensor config
		...
		app.get("/sensors/:sensor_name/tests", (request, response) => {
			...
			this.database()
				...
				.then((tests) => {
					tests = tests.map((t) => (t.path = this.#image_path_to_url(t.path)));
					response.render("sensor_tests", { tests });
				})
				.catch((err) => {
					console.log(err); // debug
					this.#redirect(response, "/sensors/all", {
						error_message: `Unable to view tests of sensor ${sensor_name}`,
					});
				});
		});
	}
	...
	#image_path_to_url(image_path) {
		if (image_path.startsWith(this.#image_folder))
			return image_path.replace(this.#image_folder, "/images");
		return null;
	}
}
