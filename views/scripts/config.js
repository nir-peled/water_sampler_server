	...
	$.getJSON(`/sensors/${sensor_name}/config.json`, (data) => {
		...
		hardware.input_selectors.forEach((port_data) => {
			$("#input-selectors-list").append(
				create_port_input(hardware.input_selectors, port_data)
			);
		});
		console.log("added input selectors"); // debug
		hardware.output_selectors.forEach((port_data) => {
			$("#output-selectors-list").append(
				create_port_input(hardware.output_selectors, port_data)
			);
		});
		console.log("added output selectors"); // debug
		hardware.operands.forEach((port_data) => {
			$("#operands-list").append(
				create_port_input(hardware.operands, port_data)
			);
		});
		console.log("added operands"); // debug
		// $("#input-selectors-list").append(
		...
		$("#plans-list").append(config.plans.map(create_plan_input));
...
function create_interrupt_input(interrupt_data) {
	let item = create_port_input(null, interrupt_data.port)
		.removeClass("port-input")
		.addClass("interrupt-input");
	$("<br>").appendTo(item);

	$("<label>").text("Seek Mode: ").appendTo(item);
	var seekModeSelect = $("<select>")
		.on("change", function () {
...
function create_step_input(steps, step_data) {
	...
	$("<button>", { class: "ref-button item-add-button" })
		.text("Add Step")
		.click(function () {
			let new_step = {};
	...
	$("<input>", { type: "text", value: step_data.name })
		.on("input", function () {
	...
	$("<input>", { type: "number", value: step_data.interval })
		.on("input", function () {
	...
	$("<input>", {
		type: "checkbox",
		checked: has_interrupt,
		class: "step-interrupt-checkbox",
	})
		.change(function () {
			if ($(this).is(":checked"))
				step_data.interrupt = $(this)
					.parent()
					.children(".step-interrupt-name")
					.val();
			else step_data.interrupt = null;
		})
		.appendTo(item);
	// step interrupt value (ignored if checkbox is unchecked)
	$("<input>", {
		type: "text",
		value: step_data.interrupt,
		class: "step-interrupt-name",
	})
		.on("input", function () {
			let interrupt_checkbox = $(this)
				.parent()
				.children(".step-interrupt-checkbox");
	...
	if (step_data.actions && Array.isArray(step_data.actions)) {
		step_data.actions.forEach(function (action_data) {
			action_list.append(
				create_step_action_input(step_data.actions, action_data)
			);
		});
	}
	...
	$("<button>", { class: "ref-button" })
		...
		.click(function () {
			...
			if (!(step_data.actions instanceof Array)) step_data.actions = [];
			step_data.actions.push(new_action);

			action_list.append(
				create_step_action_input(step_data.actions, new_action)
			);
		})
		.appendTo(item);
	$("<br>").appendTo(item);
...
function create_step_action_input(actions, action_data) {
	var action_item = $("<li>", { class: "list-group-item" });
...
function create_plan_input(plan_data) {
	...
	$("<input>", { type: "text", value: plan_data.name })
		.on("input", function () {
